    ...
    for idx, word in enumerate(wordlist):
        try:
            ...
            if response.status_code not in negative_status_codes:
                with lock:
                    print(f"\r{CYN}Progress:{GRN} [{'+'}] Found: {WHITE}/{word} [{response.status_code}]{' '*(50-len(word))} Checked {idx+1}/{total_words} words".ljust(100), flush=True, end='\n')
...
def main():
    ...
    parser.add_argument("--install", action="store_true", help="Install BAAS on your system.")
